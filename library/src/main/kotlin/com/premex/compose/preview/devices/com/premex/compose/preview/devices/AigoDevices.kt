package com.premex.compose.preview.devices

import com.premex.compose.preview.Devices
import kotlin.Any

/**
 * aigo device specifications for Android Compose previews.
 *
 * This extension provides aigo device specifications that can be used with @Preview annotations
 * in Android Compose, sourced from the Android Device Catalog maintained by Google Play Store.
 *
 * Usage:
 * ```kotlin
 * @Preview(device = Devices.Aigo.DEVICE_NAME)
 * @Composable
 * fun MyPreview() {
 *     // Your composable content
 * }
 * ```
 */
public val Devices.Aigo: Any
  get() = object {
      /** DeviceSpec(manufacturer=aigo, code=A16, width=1200, height=2000, dpi=240,
      isGoogleDevice=false).manufacturer DeviceSpec(manufacturer=aigo, code=A16, width=1200,
      height=2000, dpi=240, isGoogleDevice=false).code */
      val A16 = "spec:width=1200,height=2000,unit=px,dpi=240"

      /** DeviceSpec(manufacturer=aigo, code=A16pro, width=1200, height=2000, dpi=240,
      isGoogleDevice=false).manufacturer DeviceSpec(manufacturer=aigo, code=A16pro, width=1200,
      height=2000, dpi=240, isGoogleDevice=false).code */
      val A16PRO = "spec:width=1200,height=2000,unit=px,dpi=240"

  }
