package com.premex.compose.preview.devices

import com.premex.compose.preview.Devices
import kotlin.Any

/**
 * VOD_Horizon device specifications for Android Compose previews.
 *
 * This extension provides VOD_Horizon device specifications that can be used with @Preview
 * annotations
 * in Android Compose, sourced from the Android Device Catalog maintained by Google Play Store.
 *
 * Usage:
 * ```kotlin
 * @Preview(device = Devices.VODHorizon.DEVICE_NAME)
 * @Composable
 * fun MyPreview() {
 *     // Your composable content
 * }
 * ```
 */
public val Devices.VODHorizon: Any
  get() = object {
      /** DeviceSpec(manufacturer=VOD_Horizon, code=5G_Smart_Phone, width=1080, height=2400,
      dpi=400, isGoogleDevice=false).manufacturer DeviceSpec(manufacturer=VOD_Horizon,
      code=5G_Smart_Phone, width=1080, height=2400, dpi=400, isGoogleDevice=false).code */
      val _5G_SMART_PHONE = "spec:width=1080,height=2400,unit=px,dpi=400"

  }
