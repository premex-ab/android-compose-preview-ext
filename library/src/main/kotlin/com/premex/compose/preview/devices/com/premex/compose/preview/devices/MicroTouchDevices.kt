package com.premex.compose.preview.devices

import com.premex.compose.preview.Devices
import kotlin.Any

/**
 * MicroTouch device specifications for Android Compose previews.
 *
 * This extension provides MicroTouch device specifications that can be used with @Preview
 * annotations
 * in Android Compose, sourced from the Android Device Catalog maintained by Google Play Store.
 *
 * Usage:
 * ```kotlin
 * @Preview(device = Devices.MicroTouch.DEVICE_NAME)
 * @Composable
 * fun MyPreview() {
 *     // Your composable content
 * }
 * ```
 */
public val Devices.MicroTouch: Any
  get() = object {
      /** DeviceSpec(manufacturer=MicroTouch, code=IDC_Series, width=1080, height=1920, dpi=160,
      isGoogleDevice=false).manufacturer DeviceSpec(manufacturer=MicroTouch, code=IDC_Series,
      width=1080, height=1920, dpi=160, isGoogleDevice=false).code */
      val IDC_SERIES = "spec:width=1080,height=1920,unit=px,dpi=160"

  }
