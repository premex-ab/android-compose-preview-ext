package com.premex.compose.preview.devices

import com.premex.compose.preview.Devices
import kotlin.Any

/**
 * Volkano device specifications for Android Compose previews.
 *
 * This extension provides Volkano device specifications that can be used with @Preview annotations
 * in Android Compose, sourced from the Android Device Catalog maintained by Google Play Store.
 *
 * Usage:
 * ```kotlin
 * @Preview(device = Devices.Volkano.DEVICE_NAME)
 * @Composable
 * fun MyPreview() {
 *     // Your composable content
 * }
 * ```
 */
public val Devices.Volkano: Any
  get() = object {
      /** DeviceSpec(manufacturer=Volkano, code=VK_740_12, width=600, height=1024, dpi=160,
      isGoogleDevice=false).manufacturer DeviceSpec(manufacturer=Volkano, code=VK_740_12, width=600,
      height=1024, dpi=160, isGoogleDevice=false).code */
      val VK_740_12 = "spec:width=600,height=1024,unit=px,dpi=160"

  }
