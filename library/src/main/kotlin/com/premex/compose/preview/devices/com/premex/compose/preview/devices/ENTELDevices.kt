package com.premex.compose.preview.devices

import com.premex.compose.preview.Devices
import kotlin.Any

/**
 * ENTEL device specifications for Android Compose previews.
 *
 * This extension provides ENTEL device specifications that can be used with @Preview annotations
 * in Android Compose, sourced from the Android Device Catalog maintained by Google Play Store.
 *
 * Usage:
 * ```kotlin
 * @Preview(device = Devices.ENTEL.DEVICE_NAME)
 * @Composable
 * fun MyPreview() {
 *     // Your composable content
 * }
 * ```
 */
public val Devices.ENTEL: Any
  get() = object {
      /** DeviceSpec(manufacturer=ENTEL, code=m393vsb_entel, width=1080, height=1920, dpi=320,
      isGoogleDevice=false).manufacturer DeviceSpec(manufacturer=ENTEL, code=m393vsb_entel,
      width=1080, height=1920, dpi=320, isGoogleDevice=false).code */
      val M393VSB_ENTEL = "spec:width=1080,height=1920,unit=px,dpi=320"

  }
