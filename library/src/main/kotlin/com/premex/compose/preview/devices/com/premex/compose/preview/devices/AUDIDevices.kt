package com.premex.compose.preview.devices

import com.premex.compose.preview.Devices
import kotlin.Any

/**
 * AUDI device specifications for Android Compose previews.
 *
 * This extension provides AUDI device specifications that can be used with @Preview annotations
 * in Android Compose, sourced from the Android Device Catalog maintained by Google Play Store.
 *
 * Usage:
 * ```kotlin
 * @Preview(device = Devices.AUDI.DEVICE_NAME)
 * @Composable
 * fun MyPreview() {
 *     // Your composable content
 * }
 * ```
 */
public val Devices.AUDI: Any
  get() = object {
      /** DeviceSpec(manufacturer=AUDI, code=RSEIII, width=800, height=1280, dpi=160,
      isGoogleDevice=false).manufacturer DeviceSpec(manufacturer=AUDI, code=RSEIII, width=800,
      height=1280, dpi=160, isGoogleDevice=false).code */
      val RSEIII = "spec:width=800,height=1280,unit=px,dpi=160"

  }
