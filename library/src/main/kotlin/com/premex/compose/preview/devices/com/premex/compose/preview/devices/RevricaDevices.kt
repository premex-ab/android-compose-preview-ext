package com.premex.compose.preview.devices

import com.premex.compose.preview.Devices
import kotlin.Any

/**
 * Revrica device specifications for Android Compose previews.
 *
 * This extension provides Revrica device specifications that can be used with @Preview annotations
 * in Android Compose, sourced from the Android Device Catalog maintained by Google Play Store.
 *
 * Usage:
 * ```kotlin
 * @Preview(device = Devices.Revrica.DEVICE_NAME)
 * @Composable
 * fun MyPreview() {
 *     // Your composable content
 * }
 * ```
 */
public val Devices.Revrica: Any
  get() = object {
      /** DeviceSpec(manufacturer=Revrica, code=Uko108T, width=800, height=1280, dpi=213,
      isGoogleDevice=false).manufacturer DeviceSpec(manufacturer=Revrica, code=Uko108T, width=800,
      height=1280, dpi=213, isGoogleDevice=false).code */
      val UKO108T = "spec:width=800,height=1280,unit=px,dpi=213"

  }
