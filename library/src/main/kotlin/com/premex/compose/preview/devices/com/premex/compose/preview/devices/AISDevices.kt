package com.premex.compose.preview.devices

import com.premex.compose.preview.Devices
import kotlin.Any

/**
 * AIS device specifications for Android Compose previews.
 *
 * This extension provides AIS device specifications that can be used with @Preview annotations
 * in Android Compose, sourced from the Android Device Catalog maintained by Google Play Store.
 *
 * Usage:
 * ```kotlin
 * @Preview(device = Devices.AIS.DEVICE_NAME)
 * @Composable
 * fun MyPreview() {
 *     // Your composable content
 * }
 * ```
 */
public val Devices.AIS: Any
  get() = object {
      /** DeviceSpec(manufacturer=AIS, code=AISPLAYBOX, width=1080, height=1920, dpi=320,
      isGoogleDevice=false).manufacturer DeviceSpec(manufacturer=AIS, code=AISPLAYBOX, width=1080,
      height=1920, dpi=320, isGoogleDevice=false).code */
      val AISPLAYBOX = "spec:width=1080,height=1920,unit=px,dpi=320"

      /** DeviceSpec(manufacturer=AIS, code=B866V2-HA, width=1080, height=1920, dpi=320,
      isGoogleDevice=false).manufacturer DeviceSpec(manufacturer=AIS, code=B866V2-HA, width=1080,
      height=1920, dpi=320, isGoogleDevice=false).code */
      val B866V2_HA = "spec:width=1080,height=1920,unit=px,dpi=320"

  }
