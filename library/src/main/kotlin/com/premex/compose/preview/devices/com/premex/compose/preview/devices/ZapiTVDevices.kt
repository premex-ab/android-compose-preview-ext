package com.premex.compose.preview.devices

import com.premex.compose.preview.Devices
import kotlin.Any

/**
 * ZapiTV device specifications for Android Compose previews.
 *
 * This extension provides ZapiTV device specifications that can be used with @Preview annotations
 * in Android Compose, sourced from the Android Device Catalog maintained by Google Play Store.
 *
 * Usage:
 * ```kotlin
 * @Preview(device = Devices.ZapiTV.DEVICE_NAME)
 * @Composable
 * fun MyPreview() {
 *     // Your composable content
 * }
 * ```
 */
public val Devices.ZapiTV: Any
  get() = object {
      /** DeviceSpec(manufacturer=ZapiTV, code=SEI500PTV, width=1080, height=1920, dpi=320,
      isGoogleDevice=false).manufacturer DeviceSpec(manufacturer=ZapiTV, code=SEI500PTV, width=1080,
      height=1920, dpi=320, isGoogleDevice=false).code */
      val SEI500PTV = "spec:width=1080,height=1920,unit=px,dpi=320"

      /** DeviceSpec(manufacturer=ZapiTV, code=SEI800ZAPI, width=1080, height=1920, dpi=320,
      isGoogleDevice=false).manufacturer DeviceSpec(manufacturer=ZapiTV, code=SEI800ZAPI,
      width=1080, height=1920, dpi=320, isGoogleDevice=false).code */
      val SEI800ZAPI = "spec:width=1080,height=1920,unit=px,dpi=320"

  }
