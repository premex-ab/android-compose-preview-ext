package com.premex.compose.preview.devices

import com.premex.compose.preview.Devices
import kotlin.Any

/**
 * Goally device specifications for Android Compose previews.
 *
 * This extension provides Goally device specifications that can be used with @Preview annotations
 * in Android Compose, sourced from the Android Device Catalog maintained by Google Play Store.
 *
 * Usage:
 * ```kotlin
 * @Preview(device = Devices.Goally.DEVICE_NAME)
 * @Composable
 * fun MyPreview() {
 *     // Your composable content
 * }
 * ```
 */
public val Devices.Goally: Any
  get() = object {
      /** DeviceSpec(manufacturer=Goally, code=P3_8, width=800, height=1280, dpi=213,
      isGoogleDevice=false).manufacturer DeviceSpec(manufacturer=Goally, code=P3_8, width=800,
      height=1280, dpi=213, isGoogleDevice=false).code */
      val P3_8 = "spec:width=800,height=1280,unit=px,dpi=213"

  }
